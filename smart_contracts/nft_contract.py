...
from smart_contracts.nft_interface import NFTContractInterface


class NFTContract(NFTContractInterface):
    class Variables:
    ...
    def nft_escrow(app_id: int, asa_id: int):
        """
        Stateless smart contract to escrow NFT
        """
        return Seq(
            [
    ...
    def make_sell_offer(self, sell_price):
        ...
        app_is_active = Or(
            ...
            App.globalGet(self.Variables.app_state)
        ...
        update_state = Seq(
            [
                ...
                App.globalPut(
                    self.Variables.app_state, self.AppState.selling_in_progress
                ),
                Return(Int(1)),
            ]
        )
    ...
    def stop_sell_offer(self):
        ...
        app_is_initialized = (
            App.globalGet(self.Variables.app_state) != self.AppState.not_initialized
        )

        can_stop_selling = And(
            valid_number_of_transactions, valid_caller, app_is_initialized
        )
